...
namespace DBClient
{
    public partial class panelOperations_RemoveStorage : Form
    {
        public panelOperations_RemoveStorage()
        {
            InitializeComponent();
        }

        private void panelOperations_RemoveStorage_Load(object sender, EventArgs e)
        {
            UpdateStorageList();
        }
        ...
        private void registerButton_Click(object sender, EventArgs e)
        {
            if (storageList.SelectedItem != null)
            {
                ...
                if (MessageBox.Show("Вы уверены, что хотите удалить склад \"" + storageList.SelectedItem + "\"?\nЭта операция необратима", "Внимание", MessageBoxButtons.OKCancel) == DialogResult.OK)
                {
                    int storage_id = Queries.GetStorageID(storageList.SelectedItem.ToString());

                    Queries.RemoveStorage(storage_id);
                    MessageBox.Show("Склад \"" + storageList.SelectedItem.ToString() + "\" и записи о его содержимом удалены");
                    UpdateStorageList();
                }
            }
            else 
        ...
        private void panelOperations_RemoveStorage_VisibleChanged(object sender, EventArgs e)
        {
            ChangeStyle();
        }
    }
